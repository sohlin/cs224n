...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Randomly truncate the document to [4, int(self.block_size*7/8)] length characters.
        truncate_len = random.randint(4, int(self.block_size*7/8))
        truncated_doc = document[:min(len(document), truncate_len)]

        # 2. break the (truncated) document into three substrings.
        masked_len = random.randint(int(len(truncated_doc) / 8), int(len(truncated_doc) / 8 * 3))
        prefix_len = random.randint(1, len(truncated_doc) - masked_len - 1)
        prefix = truncated_doc[:prefix_len]
        masked = truncated_doc[prefix_len:prefix_len+masked_len]
        suffix = truncated_doc[prefix_len+masked_len:]

        # 3. Rearrange these substrings
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked + self.PAD_CHAR \
                        * (self.block_size - len(truncated_doc) - 2)
        ...
        # encode the resulting input and output strings as Long tensors and return the resulting data point.
