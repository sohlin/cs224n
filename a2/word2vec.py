...
def negSamplingLossAndGradient(
    ...
    scores = outsideVectors.dot(centerWordVec)
    sigmoid_val = sigmoid(np.r_[scores[indices[0]], -scores[indices[1:]]])
    loss = -np.sum(np.log(sigmoid_val))

    gradCenterVec = (sigmoid_val[0] - 1) * outsideVectors[outsideWordIdx] \
                    + np.sum(outsideVectors[negSampleWordIndices]
                            * ((1 - sigmoid_val[1:])[:, np.newaxis]), axis=0)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = centerWordVec * (sigmoid_val[0] - 1)
    gradOutsideVecs[negSampleWordIndices] = np.dot((1-sigmoid_val[1:])[:, np.newaxis],
                                                   centerWordVec[:, np.newaxis].T)
    ind_count = np.bincount(indices[1:])
    for i in set(indices[1:]):
        gradOutsideVecs[i] *= ind_count[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for out_word in outsideWords:
        tmp_loss, tmp_gradc, tmp_grado = word2vecLossAndGradient(
            centerWordVec,
            word2Ind[out_word],
            outsideVectors,
            dataset,
        ...
        loss += tmp_loss
        gradCenterVecs[word2Ind[currentCenterWord]] += tmp_gradc
        gradOutsideVectors += tmp_grado
    ### END YOUR CODE
